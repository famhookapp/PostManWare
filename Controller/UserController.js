  ...
  if (username) {
    ...
    conn.pgConnectionPool(function (err, clientConn, done) {
      if (err) {
        ...
        var queryStr = "SELECT * from milkdelivery_master.sp_get_usermeta($1)";
        var paramsArr = [username];
            ...
            conn.pgSelectQuery(queryStr, paramsArr, clientConn, function (err, result) {
              ...
              if (err) {
                ...
                if (result && result.rows && result.rows.length > 0) {
                  ...
                  var userDetails = {
                    ismobverified: result.rows[0].is_mobile_verified,
                    ...
                    cellno: result.rows[0].mobile_no
                  }
                  var token = jwt.sign(user, messages.tokenSec, {
                    expiresIn: '24h' // expires in 20 mins
                  });
                  res.status(200);
  ...
  if (req.body && req.body.uname) {
    ...
    conn.pgConnectionPool(function (err, clientConn, done) {
      if (err) {
        ...
        var paramsArr = [req.body.uname, schemaName];
        conn.pgSelectQuery(queryStr, paramsArr, clientConn, function (err, result) {
          res.status(200);
          if (err) {
            ...
            if (result && result.rows && result.rows.length > 0) {
              ...
              res.status(200);
              res.json({ status: messages.apiStatusSuccess });
            }
          }
          done(err);
        });
  ...
  if (req.body && req.body.mobileno) {
    ...
    conn.pgConnectionPool(function (err, clientConn, done) {
      if (err) {
        ...
        var paramsArr = [req.body.mobileno, ''];
        conn.pgSelectQuery(queryStr, paramsArr, clientConn, function (err, result) {
          res.status(200);
          if (err) {
...
exports.registerUser = function (req, res) {
  // Set the client properties that came from the POST data
  if (req.body && req.body.firstname && req.body.lastname && req.body.uname && req.body.doorkey && req.body.mobile && req.body.areaid && req.body.cityid && req.body.stateid) {
            ...
            if (result && result.rows && result.rows.length > 0) {
              if (result.rows[0]['sp_insert_userinfo'] == "-1") {
                ...
                getUserSchema(req.body.uname, function (error, resultSchema) {
                  ...
                  {
                     res.status(200);                     
                     var user = { uid: result.rows[0]['sp_insert_userinfo'], cityid: resultSchema.rows[0].city_id, schema: resultSchema.rows[0].db_name };
                      var userDetails = {
                        ismobverified: false,
                        power: false, vendor: false,
                        firstname: req.body.firstname, lastname:req.body.lastname
                      }
                      var token = jwt.sign(user, messages.tokenSec, {
                        expiresIn: '24h' // expires in 20 mins
                      });
                       req.session.userid = result.rows[0]['sp_insert_userinfo'];
                       req.session.save();
                       res.json({ status: messages.apiStatusSuccess, token: token, details: userDetails });                      
                  }
                });
...
exports.verifyUserEmail = function (req, res) {
  // Set the client properties that came from the POST data
  if (req.body && req.body.uname && req.decoded.uid) {
    logger.info("UserControl - Verify email id.");
    conn.pgConnectionPool(function (err, clientConn, done) {
      if (err) {
        console.log("UserControl - Error while connection PG" + err);
        logger.error("UserControl - Error while connection PG" + err);
        ...
        var queryStr = "SELECT * FROM milkdelivery_master.sp_update_verifyemailid($1,$2,$3);";
        var paramsArr = new Array(req.decoded.uid, req.body.uname, req.decoded.schema);
        conn.pgExecuteQuery(queryStr, paramsArr, clientConn, function (err, result) {
          res.status(200);
          if (err) {
            logger.error("UserControl - Error while email verification " + err);
            res.json({ status: messages.apiStatusError, message: messages.emailVerificationError });
          }
          else {
            //If record is saved successfully.
            if (result && result.rows && result.rows.length > 0) {
              if (result.rows[0]['sp_update_verifyemailid'] == "-1") {
                res.status(200);
                res.json({ status: messages.apiStatusError, message: messages.emailVerificationError });
              }
              else {
                res.status(200);
                res.json({ status: messages.apiStatusSuccess, message: messages.emailVerificationSuccess });
              }
            }
            else {
              res.status(200);
    ...
    logger.error("UserControl - Email verification : Missing data field");
    res.status(200);
    res.json({ status: messages.apiStatusError, message: messages.emailVerificationError });
  }
};
...
exports.verifyUserMobile = function (req, res) {
  // Set the client properties that came from the POST data
  if (req.body && req.body.mobilecode && (req.decoded && req.decoded.uid)) {
    ...
    conn.pgConnectionPool(function (err, clientConn, done) {
      if (err) {
        ...
        req.body.mobile = '0000';//remove the mobile no later.
        var decodedSchemaName = (req.decoded && req.decoded.schema) ? req.decoded.schema : '';
        var sessionSchemaName = '';//(req.session && req.session.useremail) ? req.session.useremail : '';
        var userId = (req.decoded && req.decoded.uid) ? req.decoded.uid : (req.session && req.session.tempUserSession)?req.session.tempUserSession : 0;
        if (decodedSchemaName) {
          //Check if the mobile code passed by user is correct. Change the user status.
          var queryStr = "SELECT * FROM milkdelivery_master.sp_update_verifymobile($1,$2,$3,$4,$5);";
          var paramsArr = new Array(userId, req.body.mobilecode, req.body.mobile, sessionSchemaName, decodedSchemaName);
          conn.pgExecuteQuery(queryStr, paramsArr, clientConn, function (err, result) {
            res.status(200);
            if (err) {
          ...
          logger.error("UserControl - Mobile verification : Missing schema field");
          res.status(200);
  ...
  if (req.body && req.decoded.uid) {
    ...
    conn.pgConnectionPool(function (err, clientConn, done) {
      if (err) {
        ...
        var queryStr = "SELECT * FROM milkdelivery_master.sp_insert_regeneratemobilecode($1,$2,$3,$4);";
        var paramsArr = new Array(req.decoded.uid, mobileCode, mobile, req.decoded.schema);
        conn.pgExecuteQuery(queryStr, paramsArr, clientConn, function (err, result) {
          res.status(200);
          if (err) {
...
//Forgot password generate token and send an email to user.
exports.forgotPassword = function (req, res) {
  // Set the client properties that came from the POST data
  if (req.body && req.body.email) {
    logger.info("UserControl - Forgot Password email trigger");
    conn.pgConnectionPool(function (err, clientConn, done) {
      if (err) {
        console.log("UserControl - Error while connection PG" + err);
        logger.error("UserControl - Error while connection PG" + err);
        ...
        //Remove the users current mob code and generate again.
        var queryStr = "SELECT * FROM milkdelivery_master.sp_insert_generateforgotpasswordcode($1);";
        var paramsArr = new Array(req.body.email);
        conn.pgExecuteQuery(queryStr, paramsArr, clientConn, function (err, result) {
          res.status(200);
          if (err) {
            logger.error("UserControl - Error while forgot password " + err);
            res.json({ status: messages.apiStatusError, message: messages.forgotPasswordError });
          }
          else {
            //If record is saved successfully.
            if (result && result.rows && result.rows.length > 0) {

              if(result.rows[0]['sp_insert_confirmpasswordchange'] == '')
              {
                res.status(200);
                res.json({ status: messages.apiStatusError, message: messages.forgotPasswordError });
              }
              ...
              {
                res.status(200);
                res.json({ status: messages.apiStatusSuccess, message: messages.forgotPasswordSuccess });
              }
            }
            else {
              res.status(200);
    ...
    logger.error("UserControl - Forgot password error. Email not set.");
    res.status(200);
    res.json({ status: messages.apiStatusError, message: messages.forgotPasswordError });
  }
};
...
//Forgot password generate token and send an email to user.
exports.confirmPasswordChange = function (req, res) {
  // Set the client properties that came from the POST data
  if (req.body && req.body.email && req.body.token && req.body.newpassword) {
    logger.info("UserControl - Forgot Password email trigger");
    conn.pgConnectionPool(function (err, clientConn, done) {
      if (err) {
        console.log("UserControl - Error while connection PG" + err);
        ...
        //Remove the users current mob code and generate again.
        var queryStr = "SELECT * FROM milkdelivery_master.sp_insert_confirmpasswordchange($1,$2,$3);";
        var paramsArr = new Array(req.body.email, req.body.token, req.body.newpassword);
        conn.pgExecuteQuery(queryStr, paramsArr, clientConn, function (err, result) {
          res.status(200);
          if (err) {
            logger.error("UserControl - Error while forgot password " + err);
            res.json({ status: messages.apiStatusError, message: messages.confirmPasswordError });
          }
          else {
            //If record is saved successfully.
            if (result && result.rows && result.rows.length > 0) {

              if(result.rows[0]['sp_insert_confirmpasswordchange'] == 0)
              {
                res.status(200);
                res.json({ status: messages.apiStatusError, message: messages.confirmPasswordError });
              }
              ...
              {
                res.status(200);
                res.json({ status: messages.apiStatusSuccess, message: messages.confirmPasswordSuccess });
              }
            }
            else {
              res.status(200);
    ...
    logger.error("UserControl - Forgot password error. Email not set.");
    res.status(200);
    res.json({ status: messages.apiStatusError, message: messages.confirmPasswordParametersError });
  }
};
